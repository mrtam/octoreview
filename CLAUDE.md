...
Manifest V3 Chrome extension with no service worker — all logic runs inside the popup and options pages.
...
**Two entry points**, both compiled to ES modules:
...
**Shared core**:
- `src/types.ts` — `SavedFilter`, `PullRequestResult`, `FilterCacheEntry`, `StorageShape`. Source of truth for the storage schema.
- `src/storage.ts` — wraps `chrome.storage.local` with three keys (`githubToken`, `filters`, `cacheByFilterId`). `normalizeStoredFilter` is the single trim/default normalizer; treat it as authoritative when reading or writing filters.
